...
const UpdateVaccine = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        const fetchVaccineDetails = async () => {
            try {
                ...
                const response = await authApis(token).get(endpoints["vaccines"](vaccineId));
                const vaccine = response.data;
                setFormData({
                    name: vaccine.name || "",
                    vaccine_type: vaccine.vaccine_type_name || "",
                    manufacturer: vaccine.manufacturer || "",
                    dose_count: vaccine.dose_count?.toString() || "",
                    dose_interval: vaccine.dose_interval?.toString() || "",
                    age_group: vaccine.age_group || "",
                    description: vaccine.description || "",
                    approved_date: vaccine.approved_date || "",
                    status: vaccine.active ? "Active" : "Inactive",
                });
                if (vaccine.approved_date) {
                    const [year, month, day] = vaccine.approved_date.split("-");
                ...
                Alert.alert("Lỗi", "Không thể tải thông tin vaccine.");
                navigation.goBack();
            } finally {
                setLoading(false);
            }
    ...
    const handleDelete = async () => {
        Alert.alert("Xác nhận", "Bạn có chắc muốn xóa vaccine này?", [
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Cập nhật Vaccine</Text>
            </View>
            {loading ? (
                ...
                <ScrollView contentContainerStyle={{ padding: 16 }}>
                    <View style={styles.section}>
                        ...
                        <TextInput
                            ...
                            onChangeText={(text) => setFormData({ ...formData, dose_count: text })}
                            placeholder="Ví dụ: 2"
                        ...
                        <TextInput
                            ...
                            onChangeText={(text) => setFormData({ ...formData, dose_interval: text })}
                        ...
                        <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
                            <Text style={styles.submitButtonText}>
                                {loading ? "Đang xử lý..." : "Cập nhật vaccine"}
                            </Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.deleteButton} onPress={handleDelete} disabled={loading}>
                            <Text style={styles.deleteButtonText}>Xóa Vaccine</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
            )}
